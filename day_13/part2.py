...
def compare_lists(top, bottom):
    ...
    top_str = ''.join([e for row in list(reversed(top))[:n] for e in row])
    bottom_str = ''.join([e for row in bottom[:n] for e in row])
    
    diff = [top_str[i] != bottom_str[i] for i in range(len(top_str))]
